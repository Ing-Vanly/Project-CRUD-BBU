                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{ __('Edit Unit Information') }}</h3>
                        </div>
                        <div class="card-body">
                            <form class="form-material form-horizontal"
                                action="{{ route('unit.update', $unit->id) }}" method="POST">
                                @csrf
                                @method('PUT') {{-- Required for update --}}

                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="name">{{ __('Unit Title') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="text" id="name" name="name"
                                            class="form-control @error('name') is-invalid @enderror"
                                            placeholder="{{ __('Enter unit name') }}"
                                            value="{{ old('name', $unit->name) }}" {{-- ✅ Use old() or existing value --}}
                                    ...
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <textarea id="description" name="description"
                                            ...
                                            placeholder="{{ __('Write your unit description here...') }}" rows="8">{{ old('description', $unit->description) }}</textarea> {{-- ✅ --}}
                                        @error('description')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group mt-3 text-right">
                                    ...
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save"></i> {{ __('Update Unit') }}
                                    </button>
                                </div>
                            </form>
                        </div>
